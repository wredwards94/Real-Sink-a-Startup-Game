...
public class GameHelper {
    ...
    private boolean startupFits(int[] startupCoords, int increment){
        ...
        if(increment == HORIZONTAL_INCREMENT){
            ...
            return finalLocation < GRID_SIZE; // check end isn't off bottom
        }
    } // end startupFits

    private boolean coordsAvailable(int[] startupCoods){
        for(int coord : startupCoods){ // check all potential positions
            if(grid[coord] != 0){ // if this position is taken
                // System.out.println("position: " + coord + " already taken");
                return false; // NO success
            }
        }
        return true; // if there were no clashes
    } // end coordsAvailable
    ...
    private ArrayList<String>convertCoordsToAlphaFormat(int[] startupCoords){
        ...
        for(int index : startupCoords){ // for each grid coordinate
            String alphaCoords = getAlphaCoordsFromIndex(index); // turn it into an "a0" coords
